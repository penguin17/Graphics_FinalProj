...
public:
	Tracking() { 
	model = 0; inc = GsVec(0, 0, 0); destination = GsVec(0, 0, 0); }
	SnManipulator* model;
	GsVec inc;
	GsVec destination;
};
...
void MyViewer::build_scene ()
{
	...
	manip = new SnManipulator;
	manip->child(building15);
	manip->mat() = mat * manip->mat();
	sceneObjects.insert(manip);
	rootg()->add(manip);

	mat.translation(200.0, 0.0, -400.0);
	SnModel *building16 = new SnModel;
	building16->model()->load("../building16.obj");
	manip = new SnManipulator;
	manip->child(building16);
	manip->mat() = mat * manip->mat();
	sceneObjects.insert(manip);
	rootg()->add(manip);

	mat.translation(0.0, 0.0, -500.0);
	SnModel *building17 = new SnModel;
	building17->model()->load("../building17.obj");
	manip = new SnManipulator;
	manip->child(building17);
	manip->mat() = mat * manip->mat();
	sceneObjects.insert(manip);
	rootg()->add(manip);

	mat.translation(-20.0, 0.0, -175.0);
	SnModel *building18 = new SnModel;
	building18->model()->load("../building18.obj");
	manip = new SnManipulator;
	manip->child(building18);
	manip->mat() = mat * manip->mat();
	sceneObjects.insert(manip);
	rootg()->add(manip);

	mat.translation(-600.0, 0.0, -650.0);
	SnModel *building19 = new SnModel;
	building19->model()->load("../building2=19.obj");
	manip = new SnManipulator;
	manip->child(building19);
	manip->mat() = mat * manip->mat();
	sceneObjects.insert(manip);
	rootg()->add(manip);

	mat.translation(-400.0, 0.0, -500.0);
	SnModel *building20 = new SnModel;
	building20->model()->load("../building20.obj");
	manip = new SnManipulator;
	manip->child(building20);
	manip->mat() = mat * manip->mat();
	sceneObjects.insert(manip);
	rootg()->add(manip);
	...
	carManip = new SnManipulator; carManip->draw_box(false);
	carManip->child(car);
	...
	GsVec inc = GsVec(260.0, 0.0, 370.0);
	GsVec destination = GsVec(10.0, 0.0, 370.0);
...
void MyViewer::build_wall() {
	SnModel *wall1 = new SnModel;
	wall1->model()->V.push() = GsVec(1000.0, 0.0, 1000.0);
	wall1->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall1->model()->V.push() = GsVec(-1000.0, 0.0, 1000.0);
	wall1->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall1->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall1->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall1->model()->F.push() = GsModel::Face(wall1->model()->V.size() - 1, wall1->model()->V.size() - 2, wall1->model()->V.size() - 3);
	...
	wall1->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall1->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall1->model()->F.push() = GsModel::Face(wall1->model()->V.size() - 3, wall1->model()->V.size() - 2, wall1->model()->V.size() - 1);

	GsModel::Group& g = *wall1->model()->G.push();
	g.fi = 0; // The group starts at first face,
	g.fn = wall1->model()->F.size(); // convers all faces,
	g.dmap = new GsModel::Texture; // and will be textured,
	g.dmap->fname.set("../sky.png"); // with this image.

									 // 3. Make sure the number of materials matches the number of groups:
	wall1->model()->M.push().init(); // Only the diffuse component will come from the texture,
									 //m.M.top() = ...; // so add here any other material properties you’d like.
									 // 4. Now you can add texture coordinates per vertex:
	wall1->model()->T.size(6);

	wall1->model()->T[0].set(0.0, 1.0);
	wall1->model()->T[1].set(1.0, 1.0);
	wall1->model()->T[2].set(0.0, 0.0);
	wall1->model()->T[3].set(1.0, 0.0);
	wall1->model()->T[4].set(1.0, 1.0);
	wall1->model()->T[5].set(0.0, 0.0);

	wall1->model()->set_mode(GsModel::Smooth, GsModel::PerGroupMtl);
	wall1->model()->textured = true;
	wall1->model()->translate(GsVec(0.0, -1000.0, 0.0));
	wall1->model()->rotate(GsVec(1.5, 0.0, 0.0));
	rootg()->add(wall1); // add to the scene
	render();
}
void MyViewer::build_wall2() {
	SnModel *wall2 = new SnModel;
	wall2->model()->V.push() = GsVec(1000.0, 0.0, 1000.0);
	wall2->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall2->model()->V.push() = GsVec(-1000.0, 0.0, 1000.0);
	wall2->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall2->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall2->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall2->model()->F.push() = GsModel::Face(wall2->model()->V.size() - 1, wall2->model()->V.size() - 2, wall2->model()->V.size() - 3);
	...
	wall2->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall2->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall2->model()->F.push() = GsModel::Face(wall2->model()->V.size() - 3, wall2->model()->V.size() - 2, wall2->model()->V.size() - 1);

	GsModel::Group& g = *wall2->model()->G.push();
	g.fi = 0; // The group starts at first face,
	g.fn = wall2->model()->F.size(); // convers all faces,
	g.dmap = new GsModel::Texture; // and will be textured,
	g.dmap->fname.set("../sky.png"); // with this image.
									 // 3. Make sure the number of materials matches the number of groups:
	wall2->model()->M.push().init(); // Only the diffuse component will come from the texture,
									 //m.M.top() = ...; // so add here any other material properties you’d like.
									 // 4. Now you can add texture coordinates per vertex:
	wall2->model()->T.size(6);

	wall2->model()->T[0].set(0.0, 1.0);
	wall2->model()->T[1].set(1.0, 1.0);
	wall2->model()->T[2].set(0.0, 0.0);
	wall2->model()->T[3].set(1.0, 0.0);
	wall2->model()->T[4].set(1.0, 1.0);
	wall2->model()->T[5].set(0.0, 0.0);

	wall2->model()->set_mode(GsModel::Smooth, GsModel::PerGroupMtl);
	wall2->model()->textured = true;
	wall2->model()->translate(GsVec(0.0, -1000.0, 0.0));
	wall2->model()->rotate(GsVec(0.0, 0.0, 1.5));
	rootg()->add(wall2); // add to the scene
	render();
}
void MyViewer::build_wall3() {
	SnModel *wall3 = new SnModel;
	wall3->model()->V.push() = GsVec(1000.0, 0.0, 1000.0);
	wall3->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall3->model()->V.push() = GsVec(-1000.0, 0.0, 1000.0);
	wall3->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall3->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall3->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall3->model()->F.push() = GsModel::Face(wall3->model()->V.size() - 1, wall3->model()->V.size() - 2, wall3->model()->V.size() - 3);
	...
	wall3->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall3->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall3->model()->F.push() = GsModel::Face(wall3->model()->V.size() - 3, wall3->model()->V.size() - 2, wall3->model()->V.size() - 1);

	GsModel::Group& g = *wall3->model()->G.push();
	g.fi = 0; // The group starts at first face,
	g.fn = wall3->model()->F.size(); // convers all faces,
	g.dmap = new GsModel::Texture; // and will be textured,
	g.dmap->fname.set("../sky.png"); // with this image.
									 // 3. Make sure the number of materials matches the number of groups:
	wall3->model()->M.push().init(); // Only the diffuse component will come from the texture,
									 //m.M.top() = ...; // so add here any other material properties you’d like.
									 // 4. Now you can add texture coordinates per vertex:
	wall3->model()->T.size(6);

	wall3->model()->T[0].set(0.0, 1.0);
	wall3->model()->T[1].set(1.0, 1.0);
	wall3->model()->T[2].set(0.0, 0.0);
	wall3->model()->T[3].set(1.0, 0.0);
	wall3->model()->T[4].set(1.0, 1.0);
	wall3->model()->T[5].set(0.0, 0.0);

	wall3->model()->set_mode(GsModel::Smooth, GsModel::PerGroupMtl);
	wall3->model()->textured = true;
	wall3->model()->translate(GsVec(0.0, -1000.0, 0.0));
	wall3->model()->rotate(GsVec(0.0, 0.0, -1.5));
	rootg()->add(wall3); // add to the scene
	render();
}
void MyViewer::build_wall4() {
	SnModel *wall4 = new SnModel;
	wall4->model()->V.push() = GsVec(1000.0, 0.0, 1000.0);
	wall4->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall4->model()->V.push() = GsVec(-1000.0, 0.0, 1000.0);
	wall4->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall4->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall4->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall4->model()->F.push() = GsModel::Face(wall4->model()->V.size() - 1, wall4->model()->V.size() - 2, wall4->model()->V.size() - 3);
	...
	wall4->model()->V.push() = GsVec(1000.0, 0.0, -1000.0);
	wall4->model()->N.push() = GsVec(0.0, 40.0, 0.0);
	wall4->model()->F.push() = GsModel::Face(wall4->model()->V.size() - 3, wall4->model()->V.size() - 2, wall4->model()->V.size() - 1);

	GsModel::Group& g = *wall4->model()->G.push();
	g.fi = 0; // The group starts at first face,
	g.fn = wall4->model()->F.size(); // convers all faces,
	g.dmap = new GsModel::Texture; // and will be textured,
	g.dmap->fname.set("../sky.png"); // with this image.
									 // 3. Make sure the number of materials matches the number of groups:
	wall4->model()->M.push().init(); // Only the diffuse component will come from the texture,
									 //m.M.top() = ...; // so add here any other material properties you’d like.
									 // 4. Now you can add texture coordinates per vertex:
	wall4->model()->T.size(6);

	wall4->model()->T[0].set(0.0, 1.0);
	wall4->model()->T[1].set(1.0, 1.0);
	wall4->model()->T[2].set(0.0, 0.0);
	wall4->model()->T[3].set(1.0, 0.0);
	wall4->model()->T[4].set(1.0, 1.0);
	wall4->model()->T[5].set(0.0, 0.0);

	wall4->model()->set_mode(GsModel::Smooth, GsModel::PerGroupMtl);
	wall4->model()->textured = true;
	wall4->model()->translate(GsVec(0.0, -1000.0, 0.0));
	wall4->model()->rotate(GsVec(-1.5, 0.0, 0.0));
	rootg()->add(wall4); // add to the scene
	render();
}
...
int MyViewer::handle_keyboard ( const GsEvent &e )
{
	...
	{	case GsEvent::KeyEsc : gs_exit(); return 1;
		...
		case GsEvent::KeySpace: cameraMode = !cameraMode; if (!cameraMode) { camera().eye.x = 0.0; camera().eye.y = 200.0; camera().eye.z = -100.0;
			camera().center.x = 0.0; camera().center.y = 50.0; camera().center.z = 0.0; camera().zfar = 5000; render();
		}
								else { camera().eye = eyePos; camera().center = centerPos; camera().zfar = 5000; } render(); return 1;
		default: return 1;
	}
